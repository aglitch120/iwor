...
//
//  補正:
//    - 第1希望は第1志望率が高い想定で+20%相対ボーナス
//    - 志望順位が下がるごとに-5%相対（面接対策の深さ差）
//    - 空席あり病院は+15%相対
//    - 同地域複数志望は+5%相対
//
...
export function calculateMatchProbability(
  ...
  const perHospital = wishlist.map((h, index) => {
    ...
    // 志望順位ボーナス/ペナルティ
    // 第1志望: +20%, 第2: +10%, 第3: ±0%, 第4以降: -5%ずつ
    if (index === 0) prob *= 1.20
    else if (index === 1) prob *= 1.10
    else if (index >= 3) prob *= Math.max(0.7, 1 - (index - 2) * 0.05)
