            ...
            <div className="bg-s1 rounded-xl p-3 space-y-3">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 space-y-1.5">
                ...
                <p className="text-[8px] text-amber-900/80 leading-relaxed">
                  ...
                  特に大学病院は、自大学の学生が併願先として多数登録する傾向があるため、第1志望率が低くなり数値が低く出ます。これは大学病院の研修の質とは無関係です。
                </p>
                <details className="text-[8px] text-amber-900/70">
                  ...
                  <ul className="mt-1 space-y-0.5 leading-relaxed list-disc pl-3">
                    <li><b>マッチ難易度</b>（偏差値）: 本気倍率＝（志望者数×第1志望率の5年平均）÷定員 を対数変換し偏差値化。定員4以下のプログラムは統計的ブレを補正済み。第1志望率が不明な場合は充足率実績から推定</li>
                    <li><b>穴場度</b>（0〜100点）: 空席率（40点）＋低倍率（30点）＋志望者減少トレンド（20点）＋低充足率（10点）の合算</li>
                    <li><b>志望集中度</b>（0〜100%）: 第1志望として登録した人数÷全志望者数の5年平均。高いほど本気度の高い志望者が多い</li>
                    <li><b>安定度</b>（0〜100点）: 直近3年間の充足率（マッチ者数÷定員）のばらつきの少なさ。100＝毎年安定して埋まる</li>
                  </ul>
                </details>
                <p className="text-[8px] text-amber-900/60 leading-relaxed">
                  出所: 医師臨床研修マッチング協議会（JRMP）公式発表資料（2021-2025年度）を統計処理したiwor独自指標です。一部の病院ではプログラム名称変更等によりデータの紐付けが困難なため、限られた年度のデータで算出している場合があります。数値の精度には限界があり、個別の病院の価値を評価するものではありません。
                </p>
              </div>
            </div>
      ...
      {result.perHospital.length === 0 ? (
        ...
        <div className="space-y-3">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 space-y-1.5 mt-2">
            ...
            <p className="text-[8px] text-amber-900/80 leading-relaxed">
              全受験生の実力が同一と仮定した場合に、各病院の「定員÷本気志望者数」から席を得られる確率を推定しています。実際は面接・筆記の出来で大きく変わるため、あくまで競争環境の目安です。
            </p>
            <details className="text-[8px] text-amber-900/70">
