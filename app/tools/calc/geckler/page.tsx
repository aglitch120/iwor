...
// Geckler RW 1977 — 1視野(LPF)あたりの細胞数
const data=[
  {group:'1',epithelial:'>25',neutrophil:'<10',quality:'不適',action:'唾液混入大。再提出'},
  {group:'2',epithelial:'>25',neutrophil:'10〜25',quality:'不適',action:'唾液混入大。再提出'},
  {group:'3',epithelial:'>25',neutrophil:'>25',quality:'不適',action:'唾液混入あり。培養結果の解釈に注意'},
  {group:'4',epithelial:'10〜25',neutrophil:'>25',quality:'適切',action:'良質な検体。培養適'},
  {group:'5',epithelial:'<10',neutrophil:'>25',quality:'最適',action:'最良の検体品質。培養適'},
  {group:'6',epithelial:'<25',neutrophil:'<25',quality:'—',action:'膿性でない痰。培養適否は担当医が判断'},
]
    ...
    >
      <div className="overflow-x-auto -mx-4 px-4">
        <table className="w-full text-xs border-collapse">
          <thead><tr className="bg-s1">{['群','上皮細胞(/LPF)','好中球(/LPF)','検体品質','判断'].map(h=><th key={h} className="p-2 font-bold text-tx border-b border-br text-center">{h}</th>)}</tr></thead>
          <tbody>{data.map((d,i)=>(
