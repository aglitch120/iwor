...
export default function FourTScorePage() {
  ...
  const result = useMemo(() => {
    ...
    if (total >= 6) { interpretation = `${total}点 — 高確率（HIT陽性率: Lo 2006 53%, Cuker 2012 64%）。治療は担当医が判断`; severity = 'dn' }
    else if (total >= 4) { interpretation = `${total}点 — 中間確率（HIT陽性率: Lo 2006 21%, Cuker 2012 14%）。治療は担当医が判断`; severity = 'wn' }
    else { interpretation = `${total}点 — 低確率（HIT陽性率: Lo 2006 5%, Cuker 2012 2%）。HITの可能性は低い`; }
