...
export default function CIWAArPage(){
  ...
  const result=useMemo(()=>{
    ...
    if(score>=20) return {score,severity:'dn' as const,label:'重症（≧20）: DT高リスク'}
    if(score>=10) return {score,severity:'wn' as const,label:'中等症（10-19）: 症状に基づく評価が必要'}
    return {score,severity:'ok' as const,label:'軽症（<10）'}
  },[vals])
  return(
