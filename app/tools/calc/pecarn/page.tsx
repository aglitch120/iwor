...
export default function PECARNPage(){
  ...
  const result=hasHigh?{sev:'dn' as const,label:'高リスク（ciTBI 4.3%）。最終判断は担当医による'}:hasMed?{sev:'wn' as const,label:'中リスク（ciTBI <1%）。最終判断は担当医による'}:{sev:'ok' as const,label:'低リスク（ciTBI <0.02%）。最終判断は担当医による'}
  return(
