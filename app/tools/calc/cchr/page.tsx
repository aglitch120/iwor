...
export default function CCHRPage() {
  ...
  const result = useMemo(()=>{
    ...
    if(h>0) return { score:h+m, severity:'dn' as const, label:'高リスク: 脳外科的介入が必要な損傷リスクあり' }
    if(m>0) return { score:m, severity:'wn' as const, label:'中リスク: 臨床的に重要な脳損傷リスクあり' }
    return { score:0, severity:'ok' as const, label:'低リスク: 高・中リスク因子なし' }
  },[checks])
  return(
    ...
    >
      <div className="space-y-1">
        <p className="text-xs font-bold text-dn mb-2">高リスク因子</p>
        {highRisk.map(c=><CheckItem key={c.id} id={c.id} label={c.label} checked={checks[c.id]} onChange={v=>setChecks(p=>({...p,[c.id]:v}))} />)}
        <p className="text-xs font-bold text-wn mb-2 mt-4">中リスク因子</p>
