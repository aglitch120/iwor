...
export default function CanadianCSpinePage(){
  ...
  const decide=(ct:boolean)=>setResult(ct?{label:'高・中リスク因子あり（最終判断は担当医による）',sev:'wn'}:{label:'低リスク（高・中リスク因子なし）。最終判断は担当医による',sev:'ok'})
  return(
    ...
    >
      <div className="space-y-4">
        {step===1&&!result&&(<div className="bg-s0 border border-br rounded-xl p-4"><p className="text-sm font-bold text-tx mb-3">Step 1: 高リスク因子はあるか?</p><p className="text-xs text-muted mb-3">65歳以上 / 危険な受傷機転 / 四肢のしびれ</p>
          <div className="flex gap-2"><button onClick={()=>decide(true)} className="flex-1 py-2 bg-dn/10 text-dn rounded-lg text-sm font-medium border border-dn/20">はい → 高リスク因子あり</button><button onClick={()=>setStep(2)} className="flex-1 py-2 bg-ac/10 text-ac rounded-lg text-sm font-medium border border-ac/20">いいえ → Step 2へ</button></div></div>)}
        ...
        {step===3&&!result&&(<div className="bg-s0 border border-br rounded-xl p-4"><p className="text-sm font-bold text-tx mb-3">Step 3: 能動的に頸部を左右45°回旋できるか?</p>
          <div className="flex gap-2"><button onClick={()=>decide(false)} className="flex-1 py-2 bg-ac/10 text-ac rounded-lg text-sm font-medium border border-ac/20">はい → 低リスク</button><button onClick={()=>decide(true)} className="flex-1 py-2 bg-dn/10 text-dn rounded-lg text-sm font-medium border border-dn/20">いいえ → 頸部回旋制限あり</button></div></div>)}
