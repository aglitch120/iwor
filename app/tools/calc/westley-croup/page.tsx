...
export default function WestleyPage(){
  ...
  const result=useMemo(()=>{
    ...
    if(score>=8) return {score,severity:'dn' as const,label:'重症（≧8）。治療は担当医が判断'}
    if(score>=3) return {score,severity:'wn' as const,label:'中等症（3-7）。治療は担当医が判断'}
    return {score,severity:'ok' as const,label:'軽症（≦2）。治療は担当医が判断'}
  },[vals])
  return(
