...
export default function OaklandPage() {
  ...
  const result = useMemo(() => {
    ...
    if (total <= 8) { interpretation = `${total}点 — 低リスク。帰宅判断は臨床医による` }
    else if (total <= 12) { interpretation = `${total}点 — 中リスク`; severity = 'wn' }
    else { interpretation = `${total}点 — 高リスク`; severity = 'dn' }
  ...
  return (
    <CalculatorLayout slug={toolDef.slug} title={toolDef.name} titleEn={toolDef.nameEn} description={toolDef.description}
      ...
      explanation={<div className="text-sm text-muted"><p>≦8点で低リスクと判定。治療方針は臨床医が総合的に判断する。下部消化管出血の重症度評価に使用。</p></div>}
