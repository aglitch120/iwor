...
export default function CarotidEchoPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      </div><ProPulseHint><FavoriteButton slug="interpret-carotid-echo" /></ProPulseHint></div></header>
      ...
      {results.length > 0 && <ProGate feature="interpretation" previewHeight={100}><section className="mb-8"><h2 className="text-lg font-bold text-tx mb-3">評価サマリー（{results.length}所見）</h2><div className="space-y-3">{results.map((r: any, i: number) => (<div key={i} className={`rounded-xl p-4 ${sty[r.severity as Sev]}`}><p className={`text-sm font-bold mb-1 ${tc[r.severity as Sev]}`}>{r.title}</p><p className="text-xs text-tx/80">{r.detail}</p></div>))}</div></section></ProGate>}
