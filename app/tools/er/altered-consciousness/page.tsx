...
const tree: Record<string, TreeNode> = {
  ...
  unstable: {
    ...
    result: {
      ...
      actions: [
        ...
        '簡易血糖測定（低血糖は即座に補正: 50%ブドウ糖40mL IV）',
        'ABC安定したら → Step 2へ',
      ],
  ...
  hypoglycemia: {
    ...
    result: {
      ...
      title: '低血糖 — 即座に補正',
      actions: [
        '50%ブドウ糖液 40mL（20g）IV → 数分で意識回復',
        '回復しなければ追加投与（合計40g）',
        '意識回復後: 原因検索（インスリン過量・SU薬・飲酒・副腎不全・敗血症・肝不全）',
        'SU薬による低血糖 → 遷延するため持続ブドウ糖点滴 + 入院',
        'アルコール多飲歴 → ブドウ糖投与前にチアミン100mg IV（Wernicke予防）',
        '栄養不良/アルコール依存 → チアミン100mg IV',
      ],
  ...
  meningitis: {
    ...
    result: {
      ...
      actions: [
        ...
        'Empiric therapy: CTRX 2g q12h + VCM（MRSA/PRSP カバー）+ デキサメタゾン（0.15mg/kg q6h）',
        '50歳以上/免疫不全 → ABPC 2g q4h追加（リステリア カバー）',
        '腰椎穿刺（LP）: 頭蓋内圧亢進の徴候がなければ速やかに',
        '乳頭浮腫・片麻痺・GCS低下 → LP前に頭部CT',
        'ヘルペス脳炎の可能性 → アシクロビル 10mg/kg q8h 追加',
      ],
  ...
  stroke_tpa: {
    ...
    result: {
      severity: 'critical',
      title: '急性期脳梗塞 — rt-PA静注療法（0.6mg/kg、日本基準）',
      actions: [
        '頭部CT（出血除外）→ 出血なし確認',
        'NIHSS評価',
        'rt-PA適応・禁忌チェック（出血傾向・最近の手術・重症高血圧等）',
        '血圧管理: 185/110以下に（ニカルジピン等）',
        'rt-PA 0.6mg/kg IV（10%ボーラス、残りを1hで点滴）',
        '脳神経外科/脳卒中科コンサルト',
  ...
  stroke_herniation: {
    ...
    result: {
      ...
      actions: [
        '頭部挙上30°',
        'マンニトール 20% 200mL IV（急速投与）or 高張食塩水（3% NaCl 250mL）',
        '過換気（一時的: PaCO₂ 30-35mmHg目標）— 効果は一時的',
        '脳外科緊急コンサルト → 外減圧術/血腫除去',
        '瞳孔所見・GCSを頻回に再評価',
      ],
      workup: ['頭部CT（緊急）', 'CBC・凝固・電解質', 'クロスマッチ'],
      disposition: '手術室 or ICU',
      pitfall: 'マンニトールの反復投与は浸透圧に注意（血清浸透圧>320で中止）。ステロイドは脳卒中によるヘルニアには無効（腫瘍性浮腫には有効）',
    },
  },
  ...
  aiueotips_insulin: {
    ...
    result: {
      ...
      actions: [
        ...
        '補液: 生食 1L/h × 2h → その後500mL/h（脱水補正）',
      ],
      workup: ['血糖', '血液ガス', 'ケトン体', '電解質（K!）', '血清浸透圧', 'CBC・腎機能'],
      disposition: 'ICU',
      pitfall: 'HHSはDKAより死亡率が高い（浸透圧変化による脳浮腫リスク）。血糖降下速度は50-75mg/dL/hが目標',
    },
  },
  ...
  aiueotips_endocrine: {
    ...
    result: {
      ...
      actions: [
        ...
        'Wernicke脳症（眼球運動障害・失調・意識障害の三徴）: チアミン 500mg IV × 3日',
        '副腎不全: ヒドロコルチゾン 100mg IV → 50mg q8h',
      ],
  ...
  aiueotips_overdose: {
    ...
    result: {
      ...
      actions: [
        ...
        '縮瞳→オピオイド: ナロキソン 0.4-2mg IV（反復投与可）',
        '散瞳+頻脈→交感神経刺激薬/抗コリン薬',
  ...
  aiueotips_seizure: {
    ...
    result: {
      ...
      actions: [
        'けいれん持続（≧5分）→ てんかん重積: ミダゾラム 10mg IM/IV or ジアゼパム 10mg IV',
        '5分で止まらない → ホスフェニトイン 22.5mgPE/kg IV or レベチラセタム',
