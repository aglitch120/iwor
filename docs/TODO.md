...
- [ ] **UI改修: ホームアプリ選択**: 「79種」等の数表記削除、ホーム画面と統一デザイン
- [ ] **UI改修: 臨床ツール内カード**: 1スクロール内に全表示、アプリ一覧と同デザイン
- [ ] **UI改修: ER一覧**: グリッドカードレイアウト（1行1主訴→横縦並べ）
- [ ] **UI改修: 検査読影カード**: グリッド表示、1スクロール、レスポンシブ
- [ ] **UI改修: 病棟TODO**: ヘッダー二重表示解消、レスポンシブ、モダンUI
- [ ] **UI改修: J-OSLER**: モダン・レスポンシブ化 + 初回チュートリアル
- [ ] **UI改修: 学習・論文フィード**: モダン・レスポンシブ化（teaserデザイン維持）
- [ ] **J-OSLER改修**: 「ガイド」→「病歴生成」、疾患選択→一式例表示→コピー可、モザイクteaser
- [ ] **病棟TODO→J-OSLERリンク追加**
- [ ] **薬剤ガイド: ステロイドカバー**（病態別HC補充方針目安）
- [ ] **薬剤ガイド: ステロイド副作用と対策リスト**（骨粗鬆症/糖尿病/脂質/血圧/不眠/電解質/感染症/眼科/潰瘍）
- [ ] **薬剤ガイド: 癌性疼痛コントロール**（WHO除痛ラダー等）
- [ ] **薬剤ガイド: 術前休薬リスト**
- [ ] **薬剤ガイド: 簡易懸濁可否リスト**
- [ ] **基準値早見表**（/tools/interpret/lab-values、年齢性別入力→カテゴリ別基準値、エビデンスベース）
- [ ] **検査読影: PRO部分を「推奨アクション」に変更**（モザイクteaser）
